...
def initiate_project(args):
    ...
    print "Buffer Size: " + str(args.buffersize)
    ...
    if args.evtxfile:
        print "File Selected: " + str(os.path.abspath(args.evtxfile))
        queue = validate_log_files([os.path.abspath(args.evtxfile)])

    elif args.evtxdirectory:
        print "Folder Selected: " + str(os.path.abspath(args.evtxdirectory))

        queue = prepare_files_to_process(args.evtxdirectory, folder_priorities, log_priorities, args.strict)
        if queue['count'] <= 0:
        ...
        if args.prioritiselog != "":
            print "Prioritising these logs(s):"
            for log in log_priorities:
                print "    - " + log

        if args.prioritisefolder != "":
...
def process_project(queue, args):
    ...
    for file_path in queue['files_to_process']:
        ...
        for process in range(args.cores):
            ...
            proc = Process(target=parserecord, args=(Tasks[process],file_path,RecordCount,int(args.buffersize),args.index,nodes,GlobalRecordCount,GlobalPercentageComplete,GlobalTiming,TooShortToTime,args.debug, args.authorization), name=proc_name)
            procs.append(proc)
            proc.start()
...
if __name__ == "__main__":
    ...
    fileorfolder.add_argument("--evtxfile", "-f", default=False, help="Enter the evtx file: --evtxfile security.evtx or -f security.evtx")
    fileorfolder.add_argument("--evtxdirectory", "-d", default=False, help="Enter the directory where the evtx files are stored. This can include multi-level directories. Use the full file path. ")
    ...
    parser.add_argument("--buffersize", "-b", default= 1000,help="Enter the number of records to store in buffer before sending to elasticsearch: --buffersize 1000 or -b 1000.  This defaults to 1000")
    ...
    parser.add_argument("--authorization", "-a", default='', help="Enter credentials in the format: USERNAME:PASSWORD")
    
    # Requires folder argument
    parser.add_argument("--prioritisefolder", "-p", default='', help="For multi-file processing, if your files are organised in the format identifier/evtx_file, then you can list the folders that this script should process first in the format of identifier, identifier.")
    parser.add_argument("--prioritiselog", "-l", default='', help="For multi-file processing, you can specify the names of logs you would like to process first across all files, e.g., \"security, application, system\". These names are case-insensitive with or without the .evtx extension. Files you specifiy will be processed first before other files are consiered. When combined with folder prioritisation, these files will be processed first within each folder.")
    ...
    if args.authorization != "":
        args.authorization = args.authorization.strip()
        args.authorization = base64.b64encode(args.authorization)

    # Check we're dealing with valid paths before continuing
    if args.evtxfile and not os.path.exists(args.evtxfile):
       print "The file or folder path you specified does not exist."
       print args.evtxfile
       exit()

    if args.evtxdirectory and not os.path.exists(args.evtxdirectory):
       print "The file or folder path you specified does not exist."
       exit()
